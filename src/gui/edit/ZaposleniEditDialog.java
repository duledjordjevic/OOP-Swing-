...
public class ZaposleniEditDialog extends JDialog {
	...
	public ZaposleniEditDialog(JFrame parent, KorisniciManager korisniciManager, Zaposleni editKorisnik) {
		super(parent, true);
		this.parent = parent;
...
//
			...
			txtIme.setText(editKorisnik.getIme());
			txtPrezime.setText(editKorisnik.getPrezime());
...
//			}
			...
			if(editKorisnik.getPol().equals("muski")) {
			...
			try {
				datumRodjenja = new SimpleDateFormat("dd.MM.yyyy").parse(editKorisnik.getDatum());
			...
			txtTelefon.setText(editKorisnik.getTelefon());
			txtAdresa.setText(editKorisnik.getAdresa());
			
			txtUsername.setText(editKorisnik.getUsername());
			txtLozinka.setText(editKorisnik.getLozinka());
			
			String s1 = String.valueOf(editKorisnik.getNivoStrucneSpreme());
			txtNivoStrucneSpreme.setText(s1);
			
			String s2 = String.valueOf(editKorisnik.getStaz());
			txtStaz.setText(s2);
				        	...
				        	if (!numeric & !numeric1) {
								...
								for(Korisnik k: korisniciManager.getKorisnici()) {
									if(username.equals(k.getUsername())) {
										if(!(editKorisnik.getId() == k.getId())) {
											tf = false;
										}
									}
								}
								if(tf) {
									korisniciManager.editZaposleni(editKorisnik.getId(),  ime, prezime, pol, datumRodjenja, telefon, adresa, username, lozinka, nivoStrucneSpreme, staz);
									((ZaposleniTableFrame) parent).refreshData();
									ZaposleniEditDialog.this.dispose();
								}else {
